...
int fazerTentativa(EstadoJogo *estado){
    ...
    if (todasCorretas){
        estado->venceu = 1;
        // libera próxima fase 
        if (estado->faseAtual < 5 && estado->progressoFases[estado->faseAtual] == 0){
...
void salvarProgresso(EstadoJogo *estado){
    ...
    fwrite(estado->progressoFases, sizeof(int), 5, arquivo);
    fclose(arquivo);
}
...
void carregarProgresso(EstadoJogo *estado){
    ...
    fread(estado->progressoFases, sizeof(int), 5, arquivo);
    fclose(arquivo);
}
