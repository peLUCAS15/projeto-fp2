...
void desenharMenuInicial(Texture2D imagemMenu, int *opcaoSelecionada){
    ...
    const char *subtitulo = "Wordle Edition";//escrevendo o subtitulo
    int larguraSubtitulo = MeasureText(subtitulo, 30);
    DrawText(subtitulo, (1200 - larguraSubtitulo) / 2, 190, 30, YELLOW);
    
    Vector2 posicaoMouse = GetMousePosition();
    
    const char *opcoes[] ={"INICIAR", "PALAVRAS", "INSTRUCOES", "CREDITOS", "SAIR"};//tranformando em botoes
    int yInicial = 320;
    int espacamento = 90;
    ...
    for (int i = 0; i < 5; i++){
        ...
        if (CheckCollisionPointRec(posicaoMouse, botao)){//muda a cor quando o mouse esta em cima
            cor = ORANGE;
...
const char *temasDisponiveis[] = {"Dragon Ball", "One Piece", "Harry Potter", "Resident Evil", "Silent Hill", "Final"};
const int totalTemas = 6;
...
void desenharMenuPalavras(Texture2D imagemMenu, EstadoPalavras *estado, Palavra *listaPalavras, int totalPalavras, int *opcaoSelecionada) {
    ...
    //controlando a parte das palavras
    switch (estado->estadoAtual) {
        ...
        case MENU_PALAVRAS_ADICIONAR: {
            ...
            DrawText("SALVAR", 360, 550, 25, corSalvar);
            ...
            DrawText("CANCELAR", 515, 550, 25, corCancelar);
...
int main(void){
    ...
    Texture2D imagemMenu ={0};//zerando a textura
    if (FileExists("assets/imagens/Terreno/Menu.png")){//carregando imagem
        imagemMenu = LoadTexture("assets/imagens/Terreno/Menu.png");
    }//if
    
    Texture2D imagemMenuFases ={0};//zerando a textura
    if (FileExists("assets/imagens/Terreno/MenuFases.png")){//carregando imagem
    ...
    EstadoTela telaAtual = TELA_MENU_INICIAL;//começando o jogo no menu
    ...
    while (!WindowShouldClose() && telaAtual != TELA_SAIR){
        ...
        switch (telaAtual){
            case TELA_MENU_INICIAL:{
                desenharMenuInicial(imagemMenu, &opcaoMenuSelecionada);//chamando a funçao
                
                if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)){
                    switch (opcaoMenuSelecionada){//controla a parte do menu inicial
                        case 0:
                            telaAtual = TELA_MENU;
                            break;
                        case 1:
            ...
            case TELA_INSTRUCOES:{
                desenharInstrucoes(imagemMenu);//chamando a funçao
            ...
            case TELA_CREDITOS:{
                desenharCreditos(imagemMenu);//chamando a funçao
            ...
            case TELA_PALAVRAS:{
                ...
                if (estadoPalavras.estadoAtual == MENU_PALAVRAS_PRINCIPAL) {
                    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                        switch (opcaoPalavrasSelecionada) {//controla o menu das palavras
                            case 0: // Listar todas
                            ...
                            case 1: // Adicionar
                                ...
                                memset(estadoPalavras.inputPalavra, 0, sizeof(estadoPalavras.inputPalavra));//prenche tudo com zero
                            ...
                            case 2: // Buscar
                                ...
                                memset(estadoPalavras.textoBusca, 0, sizeof(estadoPalavras.textoBusca));//prenche tudo com zero
                                break;
                            case 3: // Remover
                    ...
                    while (key > 0) {
                        if (estadoPalavras.campoSelecionado == 0 && strlen(estadoPalavras.inputPalavra) < 29) {
                            if (key >= 32 && key <= 126) {//aceitando digitar na area selecionada
                        ...
                        if (strlen(estadoPalavras.inputPalavra) > 0 && strlen(estadoPalavras.inputDica) > 0) {
                            ...
                            for (int i = 0; i < (int)strlen(estadoPalavras.inputPalavra); i++) {
                                for (int j = i + 1; j < (int)strlen(estadoPalavras.inputPalavra); j++) {
                                    if (toupper(estadoPalavras.inputPalavra[i]) == toupper(estadoPalavras.inputPalavra[j])) {//deixa tudo maiusculo
                                        temRepetidas = 1;
                                        break;
                                    }//if
                                }//for
                            ...
                            //caso a palavra nao for repetida
                            if (!temRepetidas) {
                                adicionarPalavraDireta(&listaPalavras, &totalPalavras, 
                                                     ...
                                                     (char*)temasDisponiveis[estadoPalavras.inputTemaIndex]);//chamando funçao
                                salvarPalavrasCSV(listaPalavras, totalPalavras, "assets/palavras.csv");
                                estadoPalavras.estadoAtual = MENU_PALAVRAS_PRINCIPAL;//chamando funçao
                            }//if
                        }//if
                    }//if
                    ...
                    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                        ...
                        if (CheckCollisionPointRec(mouse, botaoSalvar)) {
                            if (strlen(estadoPalavras.inputPalavra) > 0 && strlen(estadoPalavras.inputDica) > 0) {
                                ...
                                for (int i = 0; i < (int)strlen(estadoPalavras.inputPalavra); i++) {
                                    for (int j = i + 1; j < (int)strlen(estadoPalavras.inputPalavra); j++) {
                                        if (toupper(estadoPalavras.inputPalavra[i]) == toupper(estadoPalavras.inputPalavra[j])) {//deixa maiusculo
                                            temRepetidas = 1;
                                            break;
                                        }//if
                                    }//for
                    ...
                    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && strlen(estadoPalavras.textoBusca) > 0) {
                        ...
                        if (indice >= 0) {
                            ...
                            if (CheckCollisionPointRec(mouse, botaoConfirmar)) {
                                removerPalavra(&listaPalavras, &totalPalavras, indice);//chamando funçao
                                salvarPalavrasCSV(listaPalavras, totalPalavras, "assets/palavras.csv");//chamando funçao
            ...
            case TELA_CINEMATICA:{
                ...
                if (executarCinematicaIntro(&recursosCinematica, faseSelecionada, &estadosCinematicas)){
                    if(faseSelecionada != 1 && faseSelecionada != 8){
                        ...
                        if(faseSelecionada == 1) telaAtual = TELA_TRANSICAO;//primeira fase nao tem palavra
                        if(faseSelecionada == 8) telaAtual = TELA_PARABENS;//ultima fase
                    }//else
                }//if
            ...
            case TELA_JOGO:{
                ...
                Music musicaFase;
                //contra a imagem e a musica dentro do jogo de palavras
